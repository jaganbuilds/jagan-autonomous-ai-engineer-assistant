...
def test_manager_normal_flow(manager):
    ...
    mock_gateway = manager._mock_gateway
...
def test_manager_auto_tool_flow(manager):
    ...
    mock_gateway.chat.side_effect = [call_response, final_response]
...
def test_manager_confirmation_flow(manager):
    ...
    call_response = LLMResponse(text="", tool_calls=None)
    call_response.tool_calls = [tool_call]
    mock_gateway.chat.return_value = call_response
    ...
    mock_gateway.chat.side_effect = [final_response] # The resume step
...
def test_manager_job_search_flow(manager):
    ...
    tool_call.arguments = {"role": "Developer", "location": "Remote"}
    ...
    try:
        ...
        func_response_part = mock_gateway.chat.call_args_list[1][0][0]
...
def test_manager_job_match_missing_skills(manager):
    ...
    func_response_part = str(mock_gateway.chat.call_args_list[1][0][0])
...
def test_manager_job_match_tool_safety(manager):
    ...
    tool_call = ToolCall(id="t1", name="test", arguments={})
    tool_call.name = "match_job_to_candidate"
    tool_call.arguments = {"title": "Safe Job", "company": "Co", "description": "Desc"}
...
def test_manager_job_retrieval_flow(manager):
    ...
    tool_call.arguments = {"job_reference": "job 1"}
...
def test_manager_email_draft_flow(manager):
    ...
    with patch("app.llm.gateway.gateway.generate_json") as mock_json:
        mock_response = LLMResponse(text="", tool_calls=None)
        
        from app.tools.hr_email_tool import EmailDraft
        mock_json.return_value = EmailDraft(subject="Application", body="My draft", recipient="hr@example.com", job_id="job_1")
...
def test_manager_email_confirmation_flow(manager):
    ...
    tool_call.arguments = {"job_id": "job_1", "subject": "S", "body": "B", "recipient": "test@example.com"}
    ...
    # 3. Explicit reject
    mock_gateway.chat.side_effect = [final_response] # Mock LLM's response to the cancellation
...
def test_manager_email_confirmation_confirm_flow(mock_send, manager):
    ...
    mock_gateway.chat.side_effect = [final_response]
    ...
    # Verify the confirmation JSON was sent to LLM
    func_response_part = mock_gateway.chat.call_args[0][0]
    func_response_str = str(func_response_part)
...
def test_manager_discovery_flow(manager):
    ...
    tool_call.arguments = {"role": "Python"}
...
def test_manager_digest_flow(manager):
    ...
    tool_call.arguments = {"discovery_run_id": 12}
